...
struct WorldModel {
    ...
    // Per-index variants (additive to the blanket bools above): lets a query clear/drop ONE specific
    // boulder/loose-platform while leaving every other one at its load-state, so a no-strand invariant
    // can be checked per-object instead of only against the fully-cleared frontier (which, by
    // monotonicity, can't catch a single-object stranding the blanket clear wouldn't also catch).
...
inline Grid build_grid(const logic::LevelData& L, const WorldModel& wm){
    ...
    // 4. Boulders — solid unless a pound cleared them (blanket wm.boulders_broken OR this specific
    //    index is in wm.broken_boulder_idx — lets a query clear ONE boulder at a time).
    for(int i=0; i<L.boulder_count; ++i){
        if(wm.boulders_broken || wm.broken_boulder_idx.count(i)) continue;
        const logic::BoulderSpawn& b = L.boulders[i];
        if(b.tx>=0 && b.tx<L.w && b.ty>=0 && b.ty<L.h) g.solid[idx(b.tx,b.ty)] = 1;
    }
    // 5. Loose platforms — solid at their authored (load-state) row, OR dropped to their rest row when
    //    released (blanket wm.loose_dropped OR this specific index is in wm.dropped_loose_idx — lets a
    //    query drop ONE platform at a time): the first row whose tile directly below any column of the
    //    run is solid (mirrors the scene / the retired D7 fork's build_grid_stone_cleared).
    ...
    for(int i=0; i<L.loose_platform_count; ++i){
        ...
        int ty = lp.ty;
        if(wm.loose_dropped || wm.dropped_loose_idx.count(i)){
            while(ty+1 < L.h){
                bool rest = false;
