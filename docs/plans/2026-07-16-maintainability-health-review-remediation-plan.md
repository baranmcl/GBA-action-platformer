...
### Discoveries
- **Task 6.1 (latent, inert today):** the pickups/doors extraction moved DoorsSystem's archway `set_level_tile` writes to before the un-moved gates `fill_column` loop (was after). If a room ever placed a gate column at the same x (±1) as a room-door/exit archway, the last-write winner would flip. Verified ZERO such overlaps across all 25 shipped rooms. Candidate follow-up: add a door/gate column-overlap check to `tools/validate_dungeons.py` so a future room can't introduce it silently. Not fixed this cycle (no current impact).
