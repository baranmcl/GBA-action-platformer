...
// True if any tile the body's AABB overlaps is spikes (M6). Non-solid, but damaging on overlap
// (the scene skips this damage during a dash's i-frames).
inline bool spikes_overlap(const Body& b, const Tilemap& map){
    int l = Tilemap::px_to_tile(b.pos.x);
    int r = Tilemap::px_to_tile(b.pos.x + b.half_w + b.half_w - Fixed::from_int(1));
    int t = Tilemap::px_to_tile(b.pos.y);
    int bm= Tilemap::px_to_tile(b.pos.y + b.half_h + b.half_h - Fixed::from_int(1));
    for(int ty=t;ty<=bm;++ty) for(int tx=l;tx<=r;++tx) if(map.is_spikes(tx,ty)) return true;
    return false;
}
// Either damaging hazard (lava, water, or spikes) — the scene's one damage check.
inline bool hazard_overlap(const Body& b, const Tilemap& map){ return lava_overlap(b,map) || water_overlap(b,map) || spikes_overlap(b,map); }
}
