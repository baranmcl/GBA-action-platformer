...
// Phase 4), the cage/spronk rescue, and the dungeon exit. Extracted verbatim from play_room
// (Task 6.1 of the maintainability remediation) -- spawn()/update_*() bodies are unchanged
// copies of the original scene_dungeon.cpp code; only the call SITES in play_room changed
// (they now go through this object instead of inline blocks).
//
...
// happen back-to-back in the original code), then -- after PlayerSession::refresh_spell_icon()
// -- check_spronk_and_exit(). update() is split into these three methods (rather than one)
// because the shrine/heart/crystal/spronk/exit checks are NOT contiguous in the original
// per-frame loop (PlayerSession's icon refresh sits between crystals and the spronk/exit
// check), so a single update() could not occupy all of its original slots at once.
class PickupsSystem {
public:
