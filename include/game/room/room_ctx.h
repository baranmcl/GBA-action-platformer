...
// PlayerState)/lvl (map+bg view)/cam/hw/hh. Task 6.3 (enemies/terrain) adds `int& invuln` --
// EnemiesSystem's contact-damage try_hit needs it, and it's the SAME counter play_room's hazard
// try_hit + respawn already share (constructed from play_room's local `invuln` by reference, not
// a new counter). Later tasks may add cross-system refs (e.g. `GatesSystem&`) here as needed --
// do not add them speculatively before a system actually reads them.
struct Ctx {
    logic::World& world;
    logic::Player& player;
