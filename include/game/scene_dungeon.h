...
// exit is reachable. Returns Cleared on reaching the exit after the spronk is freed; Quit on
// SELECT; Restart on START (the caller re-runs the dungeon fresh + refills vitals — an
// anti-soft-lock reset).
